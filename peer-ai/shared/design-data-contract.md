...
During build, record each discrepancy as a row in the **Open Questions** table of `CONTEXT.md` at the app root, prefixed with its category: `contract`, `design`, `spec`, or `package-handoff`. When the fix needs work on the other side (a backend change, a design revision), also raise a ticket in `docs/08-issue-plan.md` or the issue tracker so it is not lost. Move the row to **Key Decisions** once it is resolved.

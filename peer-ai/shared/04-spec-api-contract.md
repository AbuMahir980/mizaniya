...
> C. I have an external API contract (a third-party payments, calendar, or messaging API, for example) that I need to work with
...
|-------------------|-------------------|-------------------|----------------------|
| List tasks with search and filters | GET /api/v1/tasks | search, status, assigneeId, page, limit | id, title, status, dueDate, assignee, commentCount |
| Task detail | GET /api/v1/tasks/:id | — | id, title, description, status, dueDate, assignee, project, comments[] |
...
|---------|----------------------|:-----:|-----|
| Calendar events for a task | GET /calendar/events?taskId= | ✅ Full | None |
| Create calendar event | POST /calendar/events | ⚠️ Partial | Missing error responses |
| Task list | — | ❌ Missing | Need from backend team |
...
|----------|:---:|:---:|---------|
| GET /tasks?search=X | ✅ Yes | Medium | Full-text search needs an index |
| GET /tasks/:id/calendar-events | ⚠️ Partial | High | Data comes from the calendar provider; needs a proxy and cache |
| POST /invoices/:id/refund | ❌ Blocked | — | No write access to the payments provider |
...
Read `docs/03-system-spec.md` and extract all endpoints needed, grouped by domain (Auth, Users, Projects, Tasks, etc.). Present them and ask the user to review.
...
|----------|------------|:-:|
| GET /tasks | PostgreSQL | ✅ Yes |
| GET /tasks/:id/calendar-events | Calendar provider API | ❌ No (proxy) |
| GET /users/:id/billing | Payments provider API | ❌ No (proxy) |
...
- Pagination → offset-based (`page`, `limit`) or cursor-based? The pagination object sits inside `data`, next to the list (as in the template)
...
**If this version has no authentication** (a local-first, single-user, or internal v1), record that in the contract's Base URL & Auth table, note what is reserved for later (e.g. `Authorization: Bearer <token>` in v2), and skip to step 5.

Otherwise, walk through step by step:
...
```json
// GET /api/v1/tasks?search=invoice&page=1&limit=25
{
  "success": true,
  "data": {
    "tasks": [
...
> "These are the standard error codes. Any domain-specific ones needed? For example, 'TASK_LOCKED' when another user is editing the task?"
...
**API Contracts v1 placement:** The recommended location for the canonical API contract is `docs/04-api-contract.md` in the GitHub repository — version-controlled alongside the code and visible to the whole team. If the team uses a shared document in their issue tracker instead, ensure the markdown version stays in sync.

**Suggested endpoint priority order** (unblock frontend development in parallel — replace the placeholders with this project's domains):
1. Auth + sessions, if this version has auth (login, token refresh, role in token payload)
2. `[PLACEHOLDER: the core resource the main screen needs — list, detail, search]`
3. `[PLACEHOLDER: the main write flow — create, update, status transitions]`
4. `[PLACEHOLDER: secondary resources — comments, attachments, history]`
5. Settings, exports, admin, and remaining endpoints

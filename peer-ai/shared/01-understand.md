...
If the user has already shared the assignment (pasted a brief, attached the email, or described the request when invoking this phase), do not ask for it again. Confirm it is complete and move on:

> "I have the brief you shared. Is that everything, or are there follow-up messages, screenshots, or documents I should see too?"

Otherwise, ask the user to share everything they have. Say something like:
...
**Wait for the user's input.** Do not proceed until you have at least the primary assignment document or description.
...
Offer the PDF-ready HTML export for `docs/01-requirements-summary.md` exactly as described under "PDF-ready doc export" in `peer-ai/shared/rules/shared.md` (one offer, `docs-pdf/` gitignored, fastest model only if the tool has a model selector). Do not repeat the wording here.

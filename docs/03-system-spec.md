...
- **While a planned amount is unsettled in a shared household, the engine uses the higher of the two figures. No exceptions.** Two wrong answers preceded this one and both are worth knowing. Design proposed *the lower*, which is backwards for protected categories — reserving less makes safe-to-spend go **up**, the exact harm it guarded against. The replacement, *whichever produces the smaller safe-to-spend*, was right arithmetically and wrong as a product: it makes **which person's number wins depend on a category type neither of them thinks about**, and on a screen whose job is looking even-handed, unpredictable is close enough to partial. *Higher* is the conservative reading of a promise, never overstates safe-to-spend (expense plans do not enter it), and is one sentence true in every case. Cost: an expense envelope warns slightly later. **Awaiting the owner's confirmation.**

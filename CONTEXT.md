...
awaits the owner:** the disagreement rule (item 40) — where design's proposal was
backwards for protected categories, **and the first replacement was wrong as well.**
Settled at *the engine uses the higher of the two, no exceptions*: a per-figure rule
would have made which person's number wins depend on a category type neither of them
thinks about, and on a screen whose job is looking even-handed, unpredictable is close
enough to partial. The lesson is the one the design-data contract exists for — the
arithmetic answer and the answer that works on a screen between two people were not the
same, and design had the better frame.

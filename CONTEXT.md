...
| 2026-09-23 | **The movement labels are everyday speech, not accounting** — *Spent*, *Received*, *Moved to savings*, *Took from savings*, and the four debt lines that already read that way. Past tense, because the owner is describing something they did: nobody says *"I made an expense."* The reasoning existed in T14's test since 22 September but had reached neither this log nor the spec, so page specs §7.4 kept the formal pair and nearly won on the strength of being written down. **Spec corrected, labels moved to `core/movement`** so one screen cannot drift from another. The design's type filter still draws the formal wording — a design follow-up | #23 |
...
| `design:` **The type filter draws the formal movement labels.** `TransactionsLight.dc.html` lists *Income · Expense · Move to savings · Take from savings*; the product's voice is *Received · Spent · Moved to savings · Took from savings*, now canonical in `core/movement` and corrected in page specs §7.4 | Open — the code ships the plain wording; the artboard needs the same four words changed. Order in the filter is unaffected and stays the design's call. Raised at #23 |

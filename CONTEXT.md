...
**Design round closed, 25 September — §H answered by design, §I answered back.** All
eleven items in §I are answered; the four figures design refused to invent are now in
`docs/seed-data.md`, and nothing there disturbs an existing assertion. **One answer
awaits the owner:** the disagreement rule (item 40), because design's proposal was
backwards for protected categories and the replacement is money behaviour with a
...
| Question | Status |
|----------|--------|
| `design:` **Which figure does the engine use while a household disagreement is unsettled?** | **Answered 25 September, and design's proposal was backwards for the case that matters.** Design proposed *the lower of the two* as conservative. Checked against `budget.ts`: a planned amount reaches safe-to-spend only through `protectedRemaining`, which counts **protected** categories only (**D1**) — so for a **Rent fund** disagreement, taking the lower figure reserves *less* and safe-to-spend goes **up**, which is the exact harm the proposal guarded against. Design's example was Food, an expense, where the amount never enters the figure at all. **The rule: the engine uses whichever figure produces the smaller safe-to-spend** — higher for a protected category, lower for an expense. One sentence, testable, derived from the rule that the app must never overstate what can be spent. **Awaiting the owner's confirmation**, since it is money behaviour with a visible consequence |
...
| `design:` **Is export reachable while signing in, before the two-budget choice?** | **Answered 25 September — yes, with no work needed.** Export reads the local snapshot through the `Repository` and writes a file: no server, no session, indifferent to whether anyone is signed in. `buildExportFile` takes a snapshot and an instant and that is all. Design's instinct was better than the architecture deserved credit for — **the escape hatch already existed and nobody had thought to put it at the one moment it is worth most** |
